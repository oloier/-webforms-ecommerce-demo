...
public partial class Admin_Consoles_Delete : System.Web.UI.Page
{
    ...
    public void DeleteItem([FriendlyUrlSegmentsAttribute(0)]int? Id)
    {
        using (_db)
        {
            var item = _db.Consoles.FirstOrDefault(m => m.consoleid == Id);

            if (item != null)
            {
                _db.Consoles.Remove(item);
                _db.SaveChanges();
            }
        }
    ...
    public OmniModels.Console GetItem([FriendlyUrlSegmentsAttribute(0)]int? Id)
    {
        if (Id == null)
        {
            return null;
        ...
        using (_db)
        {
            return _db.Consoles.FirstOrDefault(m => m.consoleid == Id);
        }
    }
