...
public partial class Admin_Reviews_Delete : System.Web.UI.Page
{
    ...
    public void DeleteItem([FriendlyUrlSegmentsAttribute(0)]int? Id)
    {
        using (_db)
        {
            var item = _db.Reviews.FirstOrDefault(m => m.reviewid == Id);
    ...
    public Review GetItem([FriendlyUrlSegmentsAttribute(0)]int? Id)
    {
        if (Id == null)
        {
            return null;
        ...
        using (_db)
        {
            return _db.Reviews.FirstOrDefault(m => m.reviewid == Id);
        }
    }
